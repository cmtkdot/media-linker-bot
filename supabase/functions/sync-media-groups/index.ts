...
serve(async (req) => {
  ...
  try {
    ...
    // Get messages with media groups that need processing
    ...
    for (const [groupId, groupMessages] of mediaGroups.entries()) {
      try {
        ...
        // Update all messages in group with analyzed content
        for (const message of groupMessages) {
          ...
          }, 0, `update_message_${message.id}_${groupId}`)
          ...
          }, 0, `queue_message_${message.id}_${groupId}`)
        }
