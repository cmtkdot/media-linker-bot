import type { TelegramMedia, GlideMutation, GlideTableSchema } from '../_shared/types.ts';

// Define the schema inline as a fallback
const defaultSchema = {
...
function validateSchema(schema: typeof GlideTableSchema | typeof defaultSchema) {
...
export function mapSupabaseToGlide(supabaseRow: TelegramMedia): GlideMutation['columnValues'] {
  ...
  const schema = validateSchema(GlideTableSchema || defaultSchema);
  ...
  const mappedData: Record<string, unknown> = {
    [schema.id.name]: supabaseRow.id,
    [schema.file_id.name]: supabaseRow.file_id,
    [schema.file_unique_id.name]: supabaseRow.file_unique_id,
    [schema.file_type.name]: supabaseRow.file_type,
    [schema.telegram_data.name]: JSON.stringify(supabaseRow.telegram_data || {}),
    [schema.glide_data.name]: JSON.stringify(supabaseRow.glide_data || {}),
    [schema.media_metadata.name]: JSON.stringify(supabaseRow.media_metadata || {}),
    [schema.media_json.name]: JSON.stringify({
  ...
  if (supabaseRow.public_url) mappedData[schema.public_url.name] = supabaseRow.public_url;
  if (supabaseRow.product_name) mappedData[schema.product_name.name] = supabaseRow.product_name;
  if (supabaseRow.product_code) mappedData[schema.product_code.name] = supabaseRow.product_code;
  if (typeof supabaseRow.quantity === 'number') mappedData[schema.quantity.name] = supabaseRow.quantity;
  if (typeof supabaseRow.processed === 'boolean') mappedData[schema.processed.name] = supabaseRow.processed;
  if (supabaseRow.processing_error) mappedData[schema.processing_error.name] = supabaseRow.processing_error;
  if (supabaseRow.last_synced_at) mappedData[schema.last_synced_at.name] = supabaseRow.last_synced_at;
  if (supabaseRow.created_at) mappedData[schema.created_at.name] = supabaseRow.created_at;
  if (supabaseRow.updated_at) mappedData[schema.updated_at.name] = supabaseRow.updated_at;
  if (supabaseRow.message_id) mappedData[schema.message_id.name] = supabaseRow.message_id;
  if (supabaseRow.caption) mappedData[schema.caption.name] = supabaseRow.caption;
  if (supabaseRow.vendor_uid) mappedData[schema.vendor_uid.name] = supabaseRow.vendor_uid;
  if (supabaseRow.purchase_date) mappedData[schema.purchase_date.name] = supabaseRow.purchase_date;
  if (supabaseRow.notes) mappedData[schema.notes.name] = supabaseRow.notes;
  if (supabaseRow.analyzed_content) {
    mappedData[schema.analyzed_content.name] = JSON.stringify(supabaseRow.analyzed_content);
  }
  if (supabaseRow.purchase_order_uid) mappedData[schema.purchase_order_uid.name] = supabaseRow.purchase_order_uid;
  if (supabaseRow.default_public_url) mappedData[schema.default_public_url.name] = supabaseRow.default_public_url;
...
export function mapGlideToSupabase(glideData: Record<string, unknown>, rowID?: string): Partial<TelegramMedia> {
  ...
  const parseJsonSafely = (value: unknown, defaultValue = {}) => {
