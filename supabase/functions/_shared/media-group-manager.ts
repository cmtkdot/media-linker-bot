export async function syncMediaGroupCaptions(
  ...
  try {
    ...
    console.log('[Media Group] Found items:', groupMedia.length);
    ...
    // Prepare update data using the most complete item and new data
    const updateData = {
      caption: caption || mostCompleteItem.caption,
      product_name: analyzedContent?.product_name || mostCompleteItem.product_name,
      product_code: analyzedContent?.product_code || mostCompleteItem.product_code,
      quantity: analyzedContent?.quantity || mostCompleteItem.quantity,
      vendor_uid: analyzedContent?.vendor_uid || mostCompleteItem.vendor_uid,
      purchase_date: analyzedContent?.purchase_date || mostCompleteItem.purchase_date,
      notes: analyzedContent?.notes || mostCompleteItem.notes,
      analyzed_content: analyzedContent || mostCompleteItem.analyzed_content
    };

    console.log('[Media Group] Update data prepared:', updateData);
